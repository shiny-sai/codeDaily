...
// 
//
// Related Topics 哈希表 数学 字符串 👍 2478 👎 0
...
class Solution {
    public int romanToInt(String s) {
        ...
        List<Integer> list = new ArrayList<>(roman.keySet());
        Collections.sort(list, Collections.reverseOrder());


        int count = 0;
        for(Integer num : list) {
            int len =roman.get(num).length();
            while(s.length() >= count +len && roman.get(num).equals(s.substring(count, count+len))) {
                result += num;
                count += len;
